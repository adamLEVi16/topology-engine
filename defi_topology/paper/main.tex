...
Survivorship, LP Representation, and a Null That Holds\par}
\vspace{1.2em}
...
run, since the relevant metapools have all delisted. Against this fragility of the
descriptive metric, one result holds under every choice we vary (survivorship, LP
...
TVL in the single archive crawl captured mid-event and repaired away. That is a
\emph{second} data-quality trap beside survivorship, one that registry-based
reconstruction must guard against. We
...
LP-resolution fork.
\item \textbf{A convention-independent null on two events} (\S\ref{sec:null}): the reconstructed
...
\subsection{The null is the part that holds}
\label{sec:null}
...
one the naive analysis would have reported as the sought-after ``topology detects the
exploit'' finding.}
...
Unlike the descriptive fraction, the null holds under every choice we can vary:
...
omitting the pools that broke: restoring all 208 dead pools, the structure is still inert.
...
of that shallow scaffold counts as ``higher-order'' is genuinely under-determined:
...
the reconstructed structure is inert, moving less than routine drift. Reaching that
...
the stable dynamics, not the fragile levels, as the finding.
