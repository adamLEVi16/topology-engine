...
contract, a three-way relationship that a pairwise graph discards and that the
\emph{nerve complex} of pool co-membership retains. Measured over Ethereum stablecoin
pools by persistent homology, the quantity is \textbf{not stable at all}. The higher-order
fraction swings from \num{0.31} to \num{0.93} across two defensible modeling choices that
are rarely reported as sensitivities. The larger driver is \textbf{LP-token
...
revives the LP-resolution comparison, which a survivor-only view cannot run at all, since
the relevant metapools have all delisted. Against that fragility, one result survives
every choice we vary. Through both the March 2023 USDC depeg and the July 2023
Curve/Vyper exploit the reconstructed structure is \textbf{inert}, moving less than
routine month-to-month drift, under either representation and on either sample. An
...
survivorship, and registry-based work has to guard against it. We read the null
economically: stablecoins are common-numéraire substitutes, so their liquidity scaffold
is shallow and does not re-wire under price stress. The methodological lesson is to report
